#!/usr/bin/env python3
"""
Run SORT Tracker on Gate3_Oct7 - DCNv2 Models
...
def run_sort_tracking(model_path, model_name, video_path, output_base_dir):
    """Run SORT tracker on Gate3_Oct7"""

    print("\n" + "="*70)
    print(f"SORT TRACKER - {model_name}")
    print("="*70)
    ...
    print(f"Tracker: botsort.yaml (SORT-based)")
    ...
    print(f"\nProcessing frames with SORT tracker...")
    print("="*70 + "\n")
    ...
    while True:
        ...
        # Run SORT tracking (using botsort.yaml which includes SORT)
        try:
            results = model.track(
                frame,
                conf=0.5,
                persist=True,
                tracker='botsort.yaml',  # BoTSORT (improved SORT)
                verbose=False
            )[0]
        except Exception as e:
    ...
    metrics_content = f"""Gate3_Oct7 SORT Tracking Metrics - {model_name}
================================================================

Configuration:
- Model: {model_name}
- Video: {gate_name}
- Tracker: BoTSORT (SORT-based)
- Confidence: 0.5
...
Avg Active Tracks:   {sum(frame_track_counts)/len(frame_track_counts):.1f}
...
Track Quality:
--------------
MT (Mostly Tracked):     {mt_ratio:.4f} ({mostly_tracked}/{total_tracks})
PT (Partially Tracked):  {pt_ratio:.4f} ({partially_tracked}/{total_tracks})
ML (Mostly Lost):        {ml_ratio:.4f} ({mostly_lost}/{total_tracks})
...
"""
    ...
    print("="*70)
    print(f"\n✓ Model: {model_name}")
    print(f"✓ Tracker: BoTSORT (SORT-based)")
...
def main():
    """Main function to run SORT tracker on all DCNv2 models"""
    print("="*70)
    print("SORT TRACKER - GATE3 DCNv2 MODELS")
    print("="*70)
    ...
    output_base_dir = 'gate3_sort_results_dcnv2'
    ...
    for model_name, model_path in models.items():
        ...
        result = run_sort_tracking(model_path, model_name, video_path, output_base_dir)
        if result:
            results_summary.append(result)

    # Summary report
    print("\n" + "="*70)
    print("SUMMARY - All DCNv2 Models with SORT Tracker")
    print("="*70)

    if results_summary:
        print(f"\n{'Model':<20} {'Tracks':<8} {'IDSW':<6} {'MT':<8} {'ML':<8} {'FPS':<8} {'Count':<6}")
        print("-"*70)
        for r in results_summary:
            print(f"{r['model']:<20} {r['total_tracks']:<8} {r['identity_switches']:<6} "
                  f"{r['mt']:<8.4f} {r['ml']:<8.4f} {r['fps']:<8.2f} {r['total_count']:<6}")

        # Save summary
        summary_file = os.path.join(output_base_dir, 'summary.txt')
        with open(summary_file, 'w') as f:
            f.write("Gate3_Oct7 SORT Tracker - DCNv2 Models Summary\n")
            f.write("="*70 + "\n\n")
            f.write(f"{'Model':<20} {'Tracks':<8} {'IDSW':<6} {'MT':<8} {'ML':<8} {'FPS':<8} {'Count':<6}\n")
            f.write("-"*70 + "\n")
            for r in results_summary:
                f.write(f"{r['model']:<20} {r['total_tracks']:<8} {r['identity_switches']:<6} "
                        f"{r['mt']:<8.4f} {r['ml']:<8.4f} {r['fps']:<8.2f} {r['total_count']:<6}\n")
