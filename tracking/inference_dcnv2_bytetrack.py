...
ByteTrack is a simple yet effective tracker that:
- Associates detections based on IoU with Kalman filter prediction
- Handles low-confidence detections for recovery
...
Usage:
    python inference_dcnv2_bytetrack.py --model <model_path> --source <video_path> --output <output_path>

Example:
    python inference_dcnv2_bytetrack.py --model weights/best.pt --source input.mp4 --output output.mp4
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='DCNv2 YOLOv8m Inference with ByteTrack')
...
def main():
    ...
    if args.output is None:
        ...
        args.output = f"{model_name}_bytetrack_output.mp4"
    ...
    print("  - Handles low-confidence detections")
    print()
    ...
    print(f"\n[4/5] Initializing ByteTrack system...")
...
Tracker Information:
--------------------
ByteTrack is a simple multi-object tracker that:
- Uses Kalman filter for motion prediction
- Associates detections with IoU matching
- Handles low-confidence detections for recovery
- Fast and efficient for real-time applications
