#!/usr/bin/env python3
"""
Run ByteTrack inference on all DCNv3 models for all gate videos

Processes all videos from /media/mydrive/GitHub/ultralytics/videos/
Outputs to /media/mydrive/GitHub/ultralytics/tracking/inference_results_dcnv3_bytetrack/{model_name}/{gate_name}/

Features:
- ByteTrack tracking (bytetrack.yaml)
...
GATE_COUNTING_LINES = {
    'Gate2_Oct7': {
        ...
        'y_positions': [1.0, 0.278]     # (1080, 300) for 1080 height - diagonal from bottom to upper-right
    },
    'Gate2.9_Oct7': {
        'x_positions': [0.0, 1.0],
    ...
    'Gate3.5_Oct7': {
        'x_positions': [0.0, 1.0],
        'y_positions': [0.547, 0.578]  # Nearly horizontal - lowered by 30 pixels from original
    },
    'Gate3.1_Oct7': {
        'x_positions': [0.885, 0.885],  # (1700, 1700) for 1920 width - vertical line
        'y_positions': [0.0, 0.194]     # (0, 210) for 1080 height
    },
    'default': {
        'x_positions': [0.0, 1.0],
...
def run_model(model_path, model_name, video_path, gate_name, output_base_dir):
    """Run ByteTrack inference for a single model on a specific video"""
    print("\n" + "="*70)
    ...
    output_video = os.path.join(output_dir, f"{gate_name}_bytetrack_output.mp4")
    ...
    print(f"ByteTrack: Counting line for {gate_name}: x={line_x_positions}, y={line_y_positions}")
    ...
    print("Starting ByteTrack processing...")
    ...
    while True:
        ...
        try:
            results = model.track(
                frame,
                conf=0.25,  # DCNv3 uses 0.25 confidence threshold
                persist=True,
        ...
        cv2.putText(extended_frame, "[DCNv3-ByteTrack]", (frame_width + 10, 55),
    ...
    metrics_file = os.path.join(output_dir, f"{gate_name}_bytetrack_metrics.txt")
    with open(metrics_file, 'w') as f:
        f.write(f"{'='*70}\n")
        f.write(f"DCNv3 ByteTrack Results - {model_name} - {gate_name}\n")
        f.write(f"{'='*70}\n\n")
...
def main():
    ...
    print("\n" + "="*70)
    print("DCNv3 Models - ByteTrack Batch Processing")
    print("="*70)
    print("\nTracker: ByteTrack (bytetrack.yaml)")
    print("  - Simple yet effective multi-object tracker")
    print("  - Kalman filter + IoU matching")
    print("  - Fast processing for real-time applications\n")

    # Configuration
    VIDEO_DIR = Path("/media/mydrive/GitHub/ultralytics/videos")
    OUTPUT_BASE_DIR = "/media/mydrive/GitHub/ultralytics/tracking/inference_results_dcnv3_bytetrack"

    # Specific DCNv3 model directories
    model_dirs = [
    ...
    # Get model files from weights/best.pt
    model_files = []
    for model_dir in model_dirs:
        weights_file = Path(model_dir) / "weights" / "best.pt"
        if weights_file.exists():
            model_files.append(weights_file)
        else:
            print(f"⚠ Warning: Model not found: {weights_file}")

    if not model_files:
        print(f"✗ No DCNv3 model files found")
        sys.exit(1)

    # Create models list as (path, name) tuples
    # Extract model name from directory (e.g., "100_dcnv3_yolov8m_fpn_second" -> "DCNv3-FPN")
    models = []
    for model_path in model_files:
        dir_name = model_path.parent.parent.name
        # Extract the variant name (fpn, full, liu, pan)
        if 'fpn' in dir_name.lower():
            ...
            model_name = dir_name
        models.append((str(model_path), model_name))

    print(f"Found {len(models)} DCNv3 model(s):")
    ...
    for video_path in video_files:
        ...
        for model_path, model_name in models:
            current_run += 1
            print(f"\n{'='*70}")
            print(f"RUN {current_run}/{total_runs} - DCNv3 ByteTrack")
            print(f"{'='*70}")
    ...
    # Print final summary
    print("\n" + "="*70)
    print("DCNv3 BYTETRACK BATCH PROCESSING COMPLETED!")
    print("="*70)
