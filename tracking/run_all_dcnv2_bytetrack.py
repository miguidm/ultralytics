#!/usr/bin/env python3
"""
Run ByteTrack inference on all DCNv2 models for all gate videos

Processes all videos from /media/mydrive/GitHub/ultralytics/videos/
Outputs to /media/mydrive/GitHub/ultralytics/tracking/inference_results_bytetrack/{model_name}/{gate_name}/
...
def run_model(model_path, model_name, video_path, gate_name, output_base_dir):
    ...
    with open(metrics_file, 'w') as f:
        f.write(f"{'='*70}\n")
        f.write(f"ByteTrack Results - {model_name} - {gate_name}\n")
        f.write(f"{'='*70}\n\n")
...
def main():
    ...
    print("\n" + "="*70)
    print("DCNv2 Models - ByteTrack Batch Processing")
    print("="*70)
    ...
    OUTPUT_BASE_DIR = "/media/mydrive/GitHub/ultralytics/tracking/inference_results_bytetrack"
    ...
    for video_path in video_files:
        ...
        for model_path, model_name in models:
            current_run += 1
            print(f"\n{'='*70}")
            print(f"RUN {current_run}/{total_runs} - ByteTrack")
            print(f"{'='*70}")
    ...
    # Print final summary
    print("\n" + "="*70)
    print("BYTETRACK BATCH PROCESSING COMPLETED!")
    print("="*70)
