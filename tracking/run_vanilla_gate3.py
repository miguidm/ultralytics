#!/usr/bin/env python3
"""
Run ByteTrack inference on Vanilla YOLOv8m model - Gate 3 videos only
"""
...
# Gate 3 counting line configuration
GATE3_COUNTING_LINE = {
    'x_positions': [0.0, 1.0],
...
def run_model_metrics_only(model_path, model_name, video_path, gate_name, output_base_dir):
    ...
    # Get Gate 3 counting line
    line_x_positions = [
        int(frame_width * GATE3_COUNTING_LINE['x_positions'][0]),
        int(frame_width * GATE3_COUNTING_LINE['x_positions'][1])
    ]
    line_y_positions = [
        int(frame_height * GATE3_COUNTING_LINE['y_positions'][0]),
        int(frame_height * GATE3_COUNTING_LINE['y_positions'][1])
    ]
    object_positions = {}
...
def main():
    """Main function to run Vanilla model on Gate 3 videos"""
    print("="*70)
    print("VANILLA YOLOV8M - GATE 3 TRACKING METRICS")
    print("="*70)
    ...
    # Summary report
    print("\n" + "="*70)
    print(f"SUMMARY - {model_name} - Gate 3 Videos")
    print("="*70)
    ...
    if results_summary:
        ...
        # Save summary
        summary_file = os.path.join(output_base_dir, model_name, 'gate3_summary.txt')
        os.makedirs(os.path.dirname(summary_file), exist_ok=True)
        with open(summary_file, 'w') as f:
            f.write(f"{model_name} - Gate 3 Tracking Metrics Summary\n")
            f.write("="*70 + "\n\n")
