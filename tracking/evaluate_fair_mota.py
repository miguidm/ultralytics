#!/usr/bin/env python3
"""
Fair MOTA Evaluation
Only evaluates predictions on frames that have ground truth annotations
Ignores predictions in non-annotated frames to avoid false FP inflation
"""
...
def parse_mot_format(mot_file, frame_filter=None):
    """
    Parse MOT format file
    frame_filter: set of frames to include (None = all frames)
    """
    ...
    with open(mot_file, 'r') as f:
        for line in f:
            ...
            # Only include frames in filter (if specified)
            if frame_filter is not None and frame_num not in frame_filter:
                continue
...
def evaluate_gate_fair(gt_file, pred_file, gate_name, model_name):
    """
    Fair evaluation: Only evaluate on GT frames
    """
    ...
    # Load predictions ONLY for GT frames
    pred_data = parse_mot_format(pred_file, frame_filter=gt_frames)
    if len(pred_data) == 0:
        print(f"  ⚠ No prediction data for {gate_name}")
        return None

    print(f"  Predictions in GT frames: {sum(len(pred_data[f]) for f in pred_data)}")

    # Find common frames
    common_frames = sorted(gt_frames & set(pred_data.keys()))
    ...
    for frame_num in common_frames:
        ...
        pred_objects = pred_data.get(frame_num, [])
...
def main():
    ...
    print("="*90)
    print("FAIR MOTA EVALUATION - Only Evaluating GT Frames")
    print("="*90)
    ...
    for gate_info in gates_to_evaluate:
        ...
        for model in models:
            ...
            print(f"\n  Evaluating {model}...")
            result = evaluate_gate_fair(gt_file, pred_file, gate, model)

            if result:
                all_results.append(result)
                print(f"    ✓ MOTA: {result['mota']:.4f} | IDF1: {result['idf1']:.4f} | "
                      f"Precision: {result['precision']:.4f} | Recall: {result['recall']:.4f}")
                print(f"      FP: {result['num_false_positives']} | FN: {result['num_misses']} | "
                      f"IDSW: {result['num_switches']}")

    # Summary table
    if all_results:
        print("\n" + "="*90)
        print("FAIR COMPARISON - ALL GATES (GT Frames Only)")
        print("="*90)
        ...
        for gate in gates:
            ...
            print(f"\n{gate.upper()}")
            print("-"*90)
            print(f"{'Model':<15} {'MOTA':<8} {'IDF1':<8} {'Precision':<10} {'Recall':<8} "
                  f"{'IDSW':<6} {'FP':<8} {'FN':<8} {'MT':<6} {'ML':<6}")
            print("-"*90)

            for r in gate_results:
                print(f"{r['model']:<15} {r['mota']:<8.4f} {r['idf1']:<8.4f} "
        ...
        print("="*90)
        print(f"{'Gate':<20} {'Best Model':<15} {'MOTA':<10} {'IDF1':<10} {'Precision':<12} {'Recall':<10}")
        print("-"*90)

        for gate in gates:
            gate_results = [r for r in all_results if r['gate'] == gate]
            best = max(gate_results, key=lambda x: x['mota'])
            print(f"{best['gate']:<20} {best['model']:<15} {best['mota']:<10.4f} {best['idf1']:<10.4f} "
                  f"{best['precision']:<12.4f} {best['recall']:<10.4f}")

        # Save results
        output_file = "fair_mota_comparison.txt"
        with open(output_file, 'w') as f:
            f.write("Fair MOTA Evaluation - GT Frames Only\n")
            f.write("="*90 + "\n\n")
            ...
            for gate in gates:
                ...
                f.write(f"\n{gate.upper()}\n")
                f.write("-"*90 + "\n")
                f.write(f"{'Model':<15} {'MOTA':<8} {'IDF1':<8} {'Precision':<10} {'Recall':<8} "
                       f"{'IDSW':<6} {'FP':<8} {'FN':<8} {'MT':<6} {'ML':<6}\n")
                f.write("-"*90 + "\n")

                for r in gate_results:
                    f.write(f"{r['model']:<15} {r['mota']:<8.4f} {r['idf1']:<8.4f} "
                           f"{r['precision']:<10.4f} {r['recall']:<8.4f} {r['num_switches']:<6} "
                           f"{r['num_false_positives']:<8} {r['num_misses']:<8} "
                           f"{r['mostly_tracked']:<6} {r['mostly_lost']:<6}\n")
