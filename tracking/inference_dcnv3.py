#!/usr/bin/env python3
"""
DCNv3 YOLOv8m Inference Script with Tracking and Counting
Based on InternImage DCNv3 - Uses SORT-based tracker (ByteTrack) for vehicle detection and counting
...
Usage:
    python inference_dcnv3.py --model <model_path> --source <video_path> --output <output_path>

Example:
    python inference_dcnv3.py --model weights/best.pt --source input.mp4 --output output.mp4
"""
import cv2
import os
import sys
import torch  # IMPORTANT: Import torch BEFORE DCNv3 to load shared libraries

# 2. Add local 'ultralytics' to Python path
# ops_dcnv3 is inside ultralytics directory, so just add the parent
sys.path.insert(0, "/media/mydrive/GitHub/ultralytics")

# 3. Verify DCNv3 module can be imported as a package
try:
    import ops_dcnv3.modules
    print(f"✓ ops_dcnv3 package loaded successfully")
except ImportError as e:
    print(f"❌ ERROR: Cannot import ops_dcnv3 module!")
    print(f"   Error: {e}")
    print(f"\n💡 Solution:")
    print(f"   1. Build DCNv3 CUDA extensions:")
    print(f"      cd /media/mydrive/GitHub/ultralytics/ops_dcnv3")
...
print("=" * 70)
print("DCNv3 (InternImage) YOLOv8m Inference Script")
print("=" * 70)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='DCNv3 YOLOv8m Inference with Tracking')
    parser.add_argument('--model', type=str,
                        default=r'/home/migui/YOLO_outputs/100_dcnv3_yolov8m_liu_second/weights/best.pt',
...
def main():
    ...
    if args.output is None:
        ...
        args.output = f"{model_name}_dcnv3_output.mp4"

    print("\n" + "="*70)
    print("DCNv3 (InternImage) YOLOv8m Inference with SORT Tracking")
    print("="*70)
    ...
    try:
        ...
        print(f"  Model uses InternImage DCNv3 deformable convolutions")
    ...
    while True:
        ...
        cv2.putText(extended_frame, "[DCNv3 + SORT]", (frame_width + 10, 55),
        ...
        if not args.no_display:
            cv2.imshow("DCNv3 YOLOv8m + SORT Tracker", extended_frame)
    ...
    output_content = f"""DCNv3 (InternImage) YOLOv8m Inference with SORT Tracking - Metrics Report
================================================================
