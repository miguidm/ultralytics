#!/usr/bin/env python3
"""
Run ByteTrack inference on Gate3_Oct7 ONLY - All models (DCNv2 and DCNv3)
With full metrics calculation
"""
...
def main():
    """Main function to run all models on Gate3_Oct7 only"""
    print("="*70)
    print("GATE3 ONLY - ALL MODELS - TRACKING METRICS EVALUATION")
    print("="*70)

    # All models to evaluate (DCNv2 and DCNv3)
    models = {
    ...
    output_base_dir = 'tracking_metrics_gate3_only'
    print(f"\nVideo: {video_path}")
    ...
    # Summary report
    print("\n" + "="*70)
    print("SUMMARY - Gate3_Oct7 - All Models")
    print("="*70)

    if results_summary:
        results_summary.sort(key=lambda x: x['mota'], reverse=True)

        print(f"\n{'Model':<15} {'IDF1':<8} {'MOTA':<8} {'IDSW':<6} {'FPS':<8} {'Count':<7} {'Det/Frame':<10}")
        print("-"*70)
        for r in results_summary:
            avg_det = r['total_detections'] / r['frames_with_detections'] if r['frames_with_detections'] > 0 else 0
            print(f"{r['model']:<15} {r['idf1']:<8.4f} {r['mota']:<8.4f} {r['idsw']:<6} {r['fps']:<8.2f} {r['total_count']:<7} {avg_det:<10.2f}")

        # Save summary
        summary_file = os.path.join(output_base_dir, 'summary.txt')
        with open(summary_file, 'w') as f:
            f.write("Gate3_Oct7 - All Models - Tracking Metrics Summary\n")
            f.write("="*70 + "\n\n")
            f.write(f"{'Model':<15} {'IDF1':<8} {'MOTA':<8} {'IDSW':<6} {'FPS':<8} {'Count':<7} {'Det/Frame':<10}\n")
            f.write("-"*70 + "\n")
            for r in results_summary:
                avg_det = r['total_detections'] / r['frames_with_detections'] if r['frames_with_detections'] > 0 else 0
                f.write(f"{r['model']:<15} {r['idf1']:<8.4f} {r['mota']:<8.4f} {r['idsw']:<6} {r['fps']:<8.2f} {r['total_count']:<7} {avg_det:<10.2f}\n")
