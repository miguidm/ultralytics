#!/usr/bin/env python3
"""
Run ByteTrack inference on all DCNv2 models - METRICS ONLY (No Video Output)
Much faster than video output version - only calculates tracking metrics

Processes all videos from /media/mydrive/GitHub/ultralytics/videos/
Outputs predictions and metrics to tracking_metrics_results/
"""
...
def main():
    """Main function to run all DCNv2 models on all videos"""
    print("="*70)
    print("DCNv2 MODELS - TRACKING METRICS EVALUATION (NO VIDEO OUTPUT)")
    print("="*70)
    ...
    # Videos to process - Gate 3 specific videos
    videos = {
    ...
    output_base_dir = 'tracking_metrics_results_dcnv2'
    ...
    # Run each model on each video
    for model_name, model_path in models.items():
        if not os.path.exists(model_path):
            print(f"\n⚠ Skipping {model_name}: model not found")
            continue
    ...
    # Summary report
    print("\n" + "="*70)
    print("SUMMARY - All DCNv2 Models - All Videos")
    print("="*70)

    if results_summary:
        results_summary.sort(key=lambda x: x['idf1'], reverse=True)

        print(f"\n{'Model':<20} {'Gate':<15} {'IDF1':<8} {'MOTA':<8} {'IDSW':<6} {'FPS':<8} {'Count':<6}")
        print("-"*70)
        for r in results_summary:
            print(f"{r['model']:<20} {r['gate']:<15} {r['idf1']:<8.4f} {r['mota']:<8.4f} {r['idsw']:<6} {r['fps']:<8.2f} {r['total_count']:<6}")

        # Save summary
        summary_file = os.path.join(output_base_dir, 'summary.txt')
        with open(summary_file, 'w') as f:
            f.write("DCNv2 Models - Tracking Metrics Summary\n")
            f.write("="*70 + "\n\n")
            f.write(f"{'Model':<20} {'Gate':<15} {'IDF1':<8} {'MOTA':<8} {'IDSW':<6} {'FPS':<8} {'Count':<6}\n")
            f.write("-"*70 + "\n")
            for r in results_summary:
                f.write(f"{r['model']:<20} {r['gate']:<15} {r['idf1']:<8.4f} {r['mota']:<8.4f} {r['idsw']:<6} {r['fps']:<8.2f} {r['total_count']:<6}\n")
